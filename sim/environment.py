...
def load_configuration(configuration_file, start_time, test_duration_seconds):
    ...
    for intersection in configuration["intersections"]:
        ...
        env = TrafficSimulationEnvHandler(
            ...
            signal_groups,
            legs,
            allowed_green_signal_combinations=intersection[
...
class TrafficSimulationEnvHandler:
        ...
        for group, color in all_signals.items():
            if color == "green":
                ...
                for other_green_lights in green_lights:
                    ...
                    if (
                        not other_green_lights
                        in self.allowed_green_signal_combinations[group]
                    ):
                        green_lights.remove(group)
                        logic_errors.append(
                            f"Invalid green light combination at time step {self.get_simulation_ticks()}: {group} and {other_green_lights}. Removed {group} from green lights."
                        )
                        break
    ...
    def _update_group_states(self, next_groups):
        ...
        for group, color in next_groups.items():
            ...
            if color == current_color:
                ...
                raise Exception(
                    "Invalid state transition at tick {self.simulation_ticks}, {current_color} -> {color}"
                )
    ...
    def _update_vehicles(self):
        ...
        for leg_name, values in self.intern_legs.items():
            ...
            for segment in segments:
                print(f"Leg: {leg_name}, segment: {segment}")
    ...
    def demo(self):
        ...
        self.set_next_signals({"B2": "green", "B1": "green"})
    ...
    def _run_one_tick(self, terminates_now=False):
        ...
        print(json.dumps(self.observable_state.model_dump(), indent=4))
